...
def badge(risk):
 s=str(risk or '').upper(); cls='good' if s in {'COMFORTABLE','FAVORED'} else 'warn' if s in {'TOSS-UP','PREDRAFT'} else 'bad'
...
def main():
 ...
 for l in cc.get('leagues',[]):
  ...
  md += [f"## {l.get('league')}",f"- {l.get('risk')} — projection {l.get('my_projection') or '—'} · margin {l.get('margin') or '—'}",f"- Action: {l.get('top_action')} — {l.get('action_detail')}",f"- Waiver: {l.get('top_waiver') or '—'} · {l.get('waiver_priority') or ''} · FAAB {l.get('faab') or '—'}",f"- Trade posture: {t.get('posture','Not active yet')}",f"- Push targets: "+(', '.join(x.get('player') for x in (t.get('push_targets') or [])[:5]) or 'none'),f"- Rebuild targets: "+(', '.join(x.get('player') for x in (t.get('rebuild_targets') or [])[:5]) or 'none'),""]
